...
serve(async (req) => {
  ...
  try {
    const { person_image_url, garment_image_url, user_id, mode, optional_details } = await req.json();
    ...
    const { data: newJob, error: insertError } = await supabase
      ...
      .insert({
        ...
        context: {
          mode: mode || 'edit',
          optional_details: optional_details || null
        }
      })
      .select('id')
      .single();
    ...
    // Asynchronously invoke the TEST worker to start the pipeline
    supabase.functions.invoke('MIRA-AGENT-worker-vto-pipeline-test', {
    ...
    console.error("[VTO Pipeline Proxy Test] Error:", error);
