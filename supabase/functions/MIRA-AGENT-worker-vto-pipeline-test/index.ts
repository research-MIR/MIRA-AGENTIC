  ...
  try {
    ...
    switch (job.status) {
      case 'pending_segmentation': {
        console.log(`[VTO Worker Test][${job_id}] Starting segmentation in '${mode}' mode...`);
        ...
        const { data: segResult, error: segError } = await supabase.functions.invoke('MIRA-AGENT-worker-segmentation-test', {
            body: { 
        ...
        await supabase.from('mira-agent-vto-pipeline-jobs-test').update({
            status: 'pending_crop',
            segmentation_result: segResult.result
        }).eq('id', job_id);
        
        supabase.functions.invoke('MIRA-AGENT-worker-vto-pipeline-test', { body: { job_id } });
        break;
      }
      case 'pending_crop': {
        console.log(`[VTO Worker Test][${job_id}] Cropping image...`);
      ...
      case 'pending_prompt_generation': {
        console.log(`[VTO Worker Test][${job_id}] Starting prompt generation in '${mode}' mode...`);
        ...
        console.log(`[VTO Worker Test][${job_id}] User preference for captioning in '${mode}' mode is '${captioningPref}'.`);
        ...
        if (captioningPref === 'none') {
            ...
            const promptWorkerName = 'MIRA-AGENT-worker-vto-advanced-prompt-generator';
      ...
      case 'pending_tryon': {
        console.log(`[VTO Worker Test][${job_id}] Starting virtual try-on...`);
        ...
        console.log(`[VTO Worker Test][${job_id}] Paused. Waiting for bitStudio job ${bitstudioJob.jobId} to complete.`);
        break;
      }
      case 'pending_composite': {
        console.log(`[VTO Worker Test][${job_id}] Starting final composite...`);
        ...
        console.log(`[VTO Worker Test][${job_id}] Pipeline complete!`);
        break;
      }
      default:
        console.log(`[VTO Worker Test] Job ${job_id} has unhandled status: ${job.status}`);
    }

    return new Response(JSON.stringify({ success: true, message: `Handled status ${job.status}` }), { headers: corsHeaders });

  } catch (error) {
    console.error(`[VTO Worker Test][${job_id}] Error:`, error);
    await supabase.from('mira-agent-vto-pipeline-jobs-test').update({ status: 'failed', error_message: error.message }).eq('id', job_id);
