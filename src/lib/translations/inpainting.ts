...
- **Use Auto-Prompt (Recommended):** For optimal results, we highly recommend leaving the "Auto-Generate" switch ON. The AI will analyze your source and reference images to create a technically perfect prompt that describes how the new garment should look within the context of the original scene (lighting, pose, etc.).
- **If Writing Manually:** If you prefer manual control, remember that you are overriding the AI's work. Your prompt must describe the object to be inserted in detail *as if it were in the scene*.
  ...
  - **✅ Good Example:** "a photorealistic blue cotton t-shirt with soft folds, under diffuse studio lighting" (Describes only the object and its lighting context)

#### 2. The Mask is Everything: Strategies for Try-On
- **Be Generous:** It's often better to mask a slightly larger area than you think you need. This gives the AI more room to blend its creation into the original image.
...
- **Garment Coverage Strategies:**
  1.  **If the new garment is smaller than the old one:** Make sure to completely cover the entire original garment to remove it completely.
  ...
  3.  **If the shapes are different:** Mask the total area that both garments would cover. This ensures the old one is removed and the new one has the necessary space.
- **Handling Arms:**
  - If the new garment does not cover the arms (e.g., a tank top), avoid masking them.
  - If the new garment covers the arms (e.g., a jacket) and you notice the arm pose is unnaturally altered, try gradually lowering the "Denoise" strength (starting from 0.95 and going down) to encourage the AI to better preserve the original pose.
