...
- **Body Type Specificity & Amplification:** If the user's brief includes a body type like "curvy" or "slim," you MUST incorporate this into the \`MODEL_DESCRIPTION\`. You need to amplify the description significantly for the AI model to understand. For a standard "curvy" request, you MUST use stronger phrasing like "a very curvy, voluptuous figure". For a request for a "very curvy" or "plus-size" model, you MUST use extremely direct and amplified terms like "a very curvy and fat model". For "slim" requests, use terms like "a very slender, slim build". The goal is to use direct, unambiguous, and even exaggerated language to ensure the generated model matches the user's intent, as the image model under-interprets subtle descriptions.
