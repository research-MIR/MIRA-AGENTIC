...
const FEATHER_RATIO = parseFloat(Deno.env.get("FEATHER_RATIO") ?? "0.03"); // 3% of min dim
...
/** Ensure we have a PNG/alpha-backed Image (JPEGs decode fine but this forces a clean RGBA buffer). */ async function toPNG(img: Image) {
  const buf = await img.encode(0); // PNG, compression 0
  return await Image.decode(buf);
}
/** Sample alpha distribution for logs */ function calcAlphaStats(img: Image, step = 50) {
  ...
  const pct = (n: number) => n / sampled * 100;
  return {
 ...
 */ function featherPatchAlpha(patch: Image, ratio = FEATHER_RATIO, minPx = FEATHER_MIN_PX, maxPx = FEATHER_MAX_PX, logPrefix = "") {
