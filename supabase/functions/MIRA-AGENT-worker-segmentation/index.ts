...
serve(async (req) => {
  ...
  try {
    ...
    // Check if the job is already completed or failed to prevent redundant work
    if (job.status === 'complete' || job.status === 'failed') {
        console.log(`[SegmentWorker][${job_id}] Job already has status '${job.status}'. Halting execution.`);
