...
async function uploadBufferToStorage(supabase, buffer, userId, filename) {
...
serve(async (req)=>{
  ...
  try {
    ...
    const expansionPx = Math.max(1, Math.round(Math.min(w, h) * 0.025));
