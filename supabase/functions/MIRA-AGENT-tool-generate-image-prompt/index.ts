...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[ArtisanEngine] Received history with ${history.length} turns for context. The last turn is from role: '${history[history.length - 1].role}'.`);
