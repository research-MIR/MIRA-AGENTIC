...
import { Wand2, Loader2, X, PlusCircle, Shirt, Users } from "lucide-react";
...
export const BatchTryOn = () => {
    ...
    const handleBatchSubmit = async () => {
        if (precisePairs.length === 0) return showError("Please add at least one precise pair to the queue.");
    
        setIsLoading(true);
        const toastId = showLoading(`Queuing ${precisePairs.length} jobs...`);
        
        const pairsForProxy = await Promise.all(precisePairs.map(async (pair) => {
        ...
        try {
            const { error } = await supabase.functions.invoke('MIRA-AGENT-proxy-batch-inpaint', {
                body: { pairs: pairsForProxy, user_id: session?.user.id }
            });
            if (error) throw error;

            dismissToast(toastId);
            showSuccess(`${precisePairs.length} jobs started successfully!`);
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-1 space-y-6">
              <Card>
                <CardHeader>
                    <CardTitle>{t('precisePairs')}</CardTitle>
                    <p className="text-sm text-muted-foreground">{t('precisePairsDescription')}</p>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <ImageUploader onFileSelect={setTempPairPerson} title={t('person')} imageUrl={tempPairPerson ? URL.createObjectURL(tempPairPerson) : null} onClear={() => setTempPairPerson(null)} icon={<Users className="h-8 w-8 text-muted-foreground" />} />
                        <ImageUploader onFileSelect={setTempPairGarment} title={t('garment')} imageUrl={tempPairGarment ? URL.createObjectURL(tempPairGarment) : null} onClear={() => setTempPairGarment(null)} icon={<Shirt className="h-8 w-8 text-muted-foreground" />} />
                    </div>
                    <div>
              ...
              <Button size="lg" className="w-full" onClick={handleBatchSubmit} disabled={isLoading || precisePairs.length === 0}>
                ...
                {t('startBatchTryOn')} ({precisePairs.length})
              </Button>
            </div>
            <div className="lg:col-span-2">
                <Card className="min-h-[75vh]">
                    <CardHeader><CardTitle>{t('batchQueue')} ({precisePairs.length})</CardTitle></CardHeader>
                    <CardContent>
                        <ScrollArea className="h-[65vh]">
                            <div className="space-y-2 pr-4">
                                {precisePairs.map((pair, i) => (
                                    <div key={i} className="flex gap-2 items-center bg-muted p-2 rounded-md">
                                        <img src={URL.createObjectURL(pair.person)} className="w-16 h-16 object-cover rounded-md" alt="Person" />
                                        <PlusCircle className="h-5 w-5 text-muted-foreground flex-shrink-0" />
                                        <img src={URL.createObjectURL(pair.garment)} className="w-16 h-16 object-cover rounded-md" alt="Garment" />
                                        <p className="text-xs text-muted-foreground flex-1 truncate italic pl-2">"{pair.appendix || 'No specific instructions'}"</p>
                                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setPrecisePairs(p => p.filter((_, index) => index !== i))}>
                                            <X className="h-4 w-4" />
                                        </Button>
                                    </div>
                                ))}
