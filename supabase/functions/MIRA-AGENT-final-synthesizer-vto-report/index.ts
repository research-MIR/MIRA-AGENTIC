...
serve(async (req) => {
  ...
  try {
    ...
    if (!chunks || chunks.length === 0) {
      throw new Error("No completed report chunks found to synthesize.");
    }

    const chunkReports = chunks.map(c => c.result_data).filter(Boolean);
    if (chunkReports.length === 0) {
        throw new Error("Completed chunks were found, but they contained no result data.");
    }

    console.log(`${logPrefix} Found ${chunkReports.length} completed chunks. Starting final synthesis.`);
    ...
    await supabase.from('mira-agent-vto-packs-jobs').update({
        synthesis_report: `# Analysis Failed\n\nAn error occurred during the final synthesis: ${error.message}`
    }).eq('id', pack_id);
