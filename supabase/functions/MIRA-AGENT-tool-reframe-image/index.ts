...
serve(async (req) => {
  ...
  try {
    ...
    const { base_image_url, mask_image_url, final_prompt_used, dilation, steps, count } = job.context;
    const prompt = final_prompt_used || "A high quality photorealistic image.";
    
    console.log(`${logPrefix} Using final prompt: "${prompt}"`);
    ...
    // Create the final, perfectly aligned base image for the API
    const finalCanvas = createCanvas(maskImage.width(), maskImage.height());
          ...
          referenceImages: [
            {
              ...
              referenceId: 1,
              referenceImage: {
                bytesBase64Encoded: finalBaseImageB64
              }
            },
            {
              ...
              referenceId: 2,
              referenceImage: {
                bytesBase64Encoded: finalMaskImageB64
              },
              maskImageConfig: {
                maskMode: "MASK_MODE_USER_PROVIDED",
                dilation: dilation || 0.03
              }
            }
          ]
        }
