...
1.  **Forensic Garment Comparison:** Visually compare the garment in the FINAL RESULT against the REFERENCE GARMENT.
    - **CRITICAL CONTEXT RULE: You MUST differentiate between external, visible design elements (like a chest logo or graphic print) and internal, non-visible elements (like a brand tag inside the collar or on a waistband). The absence of an internal tag that would not be visible when worn is NOT a failure and should not negatively impact any score or the overall pass/fail decision.**
    - **GARMENT TYPE INTEGRITY: Your primary goal is to verify the garment is of the same fundamental type and style (e.g., a hoodie is a hoodie, a t-shirt is a t-shirt). Minor variations in fit (e.g., 'oversized' vs. 'standard') are acceptable and should be noted in your analysis, but they should not cause an automatic failure unless they fundamentally change the garment's category (e.g., a jacket becomes a vest).**
...
6.  **Nuanced Pass/Fail Logic (CRITICAL):**
    ...
    - **FAIL IF:** The body is unnaturally or unrealistically altered (\`failure_category: "body_distortion"\`), there are severe anatomical errors (\`failure_category: "anatomical_error"\`), the lighting/blending is poor (\`failure_category: "quality_issue"\`), or the garment fit is fundamentally wrong (\`failure_category: "fitting_issue"\`).
...
async function downloadAndEncodeImageAsPart(supabase: SupabaseClient, publicUrl: string, label: string): Promise<Part[]> {
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`${logPrefix} Stage 0: Downloading and encoding resized images in parallel...`);
    const [personImageParts, referenceGarmentParts, finalResultParts] = await Promise.all([
        downloadAndEncodeImageAsPart(supabase, sourcePersonImageUrl, "SOURCE PERSON"),
        downloadAndEncodeImageAsPart(supabase, vtoJob.source_garment_image_url, "REFERENCE GARMENT"),
        downloadAndEncodeImageAsPart(supabase, vtoJob.final_image_url, "FINAL RESULT")
    ]);
