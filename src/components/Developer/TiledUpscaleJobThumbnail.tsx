...
interface Job {
  id: string;
  status: 'tiling' | 'generating' | 'compositing' | 'complete' | 'failed' | 'queued' | 'processing' | 'pending' | 'queued_for_generation';
...
export const TiledUpscaleJobThumbnail = ({ job, onClick, isSelected }: Props) => {
  ...
  return (
    <button 
      onClick={onClick} 
      className={cn("border-2 rounded-lg p-1 flex-shrink-0 w-full h-full", isSelected ? "border-primary" : "border-transparent")}
      role="button"
      tabIndex={0}
