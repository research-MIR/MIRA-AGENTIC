...
import { Loader2, Image as ImageIcon, Sparkles, Wand2, Info, UploadCloud, X } from "lucide-react";
...
const FileUploader = ({ onFileSelect, children }: { onFileSelect: (files: FileList | null) => void, children: React.ReactNode }) => {
  ...
  return (
    <div className="p-4 border-2 border-dashed rounded-lg text-center cursor-pointer hover:border-primary" onClick={() => inputRef.current?.click()}>
      {children}
      <Input ref={inputRef} type="file" multiple className="hidden" accept="image/*" onChange={(e) => onFileSelect(e.target.files)} />
    </div>
  );
};
    ...
    <div className="p-4 md:p-8 h-screen overflow-y-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Textarea id="prompt" value={state.prompt} onChange={(e) => state.setField('prompt', e.target.value)} placeholder={t('promptPlaceholderGenerator')} />
              </div>
              <div>
                <Label htmlFor="negative-prompt">{t('negativePrompt')}</Label>
                <Textarea id="negative-prompt" value={state.negativePrompt} onChange={(e) => state.setField('negativePrompt', e.target.value)} placeholder={t('negativePromptPlaceholder')} />
              </div>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <FileUploader onFileSelect={(files) => state.handleFileSelect('garment', files)}>
                  <UploadCloud className="mx-auto h-8 w-8 text-muted-foreground" />
                  <p className="mt-2 text-sm">{t('uploadFiles')}</p>
                </FileUploader>
              ...
              <div className="space-y-2">
                ...
                {state.styleReferenceFile ? (
                  ...
                  <FileUploader onFileSelect={(files) => state.handleFileSelect('style', files)}>
                    <UploadCloud className="mx-auto h-8 w-8 text-muted-foreground" />
                    <p className="mt-2 text-sm">{t('uploadAFile')}</p>
                  </FileUploader>
                )}
              </div>
            </CardContent>
          ...
          <Button size="lg" className="w-full" onClick={handleGenerate} disabled={state.isLoading}>
            {state.isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}
            {t('generate')}
          </Button>
        </div>
