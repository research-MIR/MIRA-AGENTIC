...
export const RefinePackModal = ({ isOpen, onClose, onRefine, isLoading, packName }: RefinePackModalProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t('refinePackTitle')}: {packName}</DialogTitle>
          <DialogDescription>{t('refinePackDescription')}</DialogDescription>
        </DialogHeader>
          ...
          <RadioGroup value={scope} onValueChange={(value: RefineScope) => setScope(value)}>
            <div className="flex items-start space-x-3 p-4 border rounded-md has-[:checked]:border-primary">
              ...
              <Label htmlFor="successful_only" className="font-normal w-full cursor-pointer">
                <span className="font-semibold block">{t('refineSuccessfulOnly')}</span>
                <span className="text-sm text-muted-foreground">{t('refineSuccessfulOnlyDesc')}</span>
              </Label>
            </div>
            <div className="flex items-start space-x-3 p-4 border rounded-md has-[:checked]:border-primary">
              ...
              <Label htmlFor="all_completed" className="font-normal w-full cursor-pointer">
                <span className="font-semibold block">{t('refineAllCompleted')}</span>
                <span className="text-sm text-muted-foreground">{t('refineAllCompletedDesc')}</span>
              </Label>
            </div>
          </RadioGroup>
        </div>
