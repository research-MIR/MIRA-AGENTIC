...
export const BatchInpaintPro = () => {
    ...
    const handleBatchSubmit = async () => {
        let pairs: { person: File, garment: File, appendix?: string, is_helper_enabled: boolean }[] = [];
        if (batchMode === 'one-garment') {
          ...
          pairs = batchPersonFiles.map(person => ({ person, garment: batchGarmentFile, appendix: generalAppendix, is_helper_enabled: isHelperEnabled }));
          ...
          for (let i = 0; i < numPairs; i++) {
            pairs.push({ person: shuffledPeople[i], garment: shuffledGarments[i], appendix: generalAppendix, is_helper_enabled: isHelperEnabled });
          }
        } else if (batchMode === 'precise') {
          if (precisePairs.length === 0) return showError("Please add at least one precise pair.");
          pairs = precisePairs.map(p => ({...p, is_helper_enabled: p.isHelperEnabled}));
        }
        ...
        try {
            const uploadPromises = pairs.map(async (pair) => {
                const [person_url, garment_url] = await Promise.all([
                    uploadFile(pair.person, 'person'),
                    uploadFile(pair.garment, 'garment')
                ]);
                return { person_url, garment_url, appendix: pair.appendix, is_helper_enabled: pair.is_helper_enabled };
            });
