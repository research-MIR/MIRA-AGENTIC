...
const STALLED_VTO_WORKER_CATCH_ALL_THRESHOLD_SECONDS = 55; // Adjusted as per request
...
serve(async (req)=>{
  ...
  try {
    ...
    const recoverStalledJobs = async (tableName: string, statuses: string[], thresholdSeconds: number, workerName: string, idColumnName = 'id', payloadKey = 'job_id', extraFilters: Record<string, any> = {})=>{
      ...
      let query = supabase.from(tableName).select(idColumnName).in('status', statuses).lt('updated_at', threshold);
      ...
      if (stalledJobs && stalledJobs.length > 0) {
        ...
        const recoveryPromises = stalledJobs.map(async (job)=>{
          ...
          const { count, error: updateError } = await supabase.from(tableName).update({
            updated_at: new Date().toISOString()
          }).eq(idColumnName, jobId).lt('updated_at', threshold);
          if (updateError) {
          ...
          if (count && count > 0) {
            ...
            await supabase.functions.invoke(workerName, {
              body: {
                [payloadKey]: jobId
              }
            });
    ...
    try {
      ...
      await recoverStalledJobs('mira-agent-mask-aggregation-jobs', [
        'aggregating',
        'compositing'
      ], STALLED_AGGREGATION_THRESHOLD_SECONDS, 'MIRA-AGENT-compositor-segmentation', 'id', 'job_id');
    } catch (e) {
      console.error(`[Watchdog-BG][${requestId}] Task 3 (Stalled Segmentation Aggregation) failed:`, e.message);
    }
    try {
    ...
    try {
      ...
      if (stalledPairJobs && stalledPairJobs.length > 0) {
        ...
        const retryPromises = stalledPairJobs.map(async (job)=>{
          const { count, error: updateError } = await supabase.from('mira-agent-batch-inpaint-pair-jobs').update({
            updated_at: new Date().toISOString()
          }).eq('id', job.id).lt('updated_at', pairJobThreshold);
          if (updateError || !count) return; // Skip if error or already handled
          if (job.status === 'segmenting') {
            await supabase.functions.invoke('MIRA-AGENT-worker-batch-inpaint', {
              body: {
                pair_job_id: job.id
              }
            });
      ...
      console.error(`[Watchdog-BG][${requestId}] Task 5 (Stalled Pair Jobs) failed:`, e.message);
    }
    try {
      ...
      console.error(`[Watchdog-BG][${requestId}] Task 6/7 (VTO Catch-All) failed:`, e.message);
    }
    try {
      ...
      console.error(`[Watchdog-BG][${requestId}] Task 8 (VTO Concurrency & Start) failed:`, e.message);
    }
    try {
      ...
      if (awaitingReframeJobs && awaitingReframeJobs.length > 0) {
        const reframeCheckPromises = awaitingReframeJobs.map(async (vtoJob)=>{
          ...
          if (reframeJob?.status === 'complete') await supabase.functions.invoke('MIRA-AGENT-worker-vto-pack-item', {
            body: {
              pair_job_id: vtoJob.id,
              reframe_result_url: reframeJob.final_result?.images?.[0]?.publicUrl
            }
          });
          else if (reframeJob?.status === 'failed') await supabase.from('mira-agent-bitstudio-jobs').update({
            status: 'failed',
      ...
      console.error(`[Watchdog-BG][${requestId}] Task 9 (VTO Reframe Check) failed:`, e.message);
    }
    try {
      ...
      if (awaitingRecontextJobs && awaitingRecontextJobs.length > 0) {
        const recontextCheckPromises = awaitingRecontextJobs.map(async (recontextJob)=>{
          ...
          if (!reframeJobId) return;
          const { data: reframeJob } = await supabase.from('mira-agent-jobs').select('status, final_result, error_message').eq('id', reframeJobId).single();
          if (reframeJob?.status === 'complete') await supabase.from('mira-agent-jobs').update({
            status: 'complete',
            final_result: reframeJob.final_result
          }).eq('id', recontextJob.id);
          else if (reframeJob?.status === 'failed') await supabase.from('mira-agent-jobs').update({
            status: 'failed',
            error_message: `Delegated reframe job failed: ${reframeJob.error_message}`
          }).eq('id', recontextJob.id);
        });
      ...
      console.error(`[Watchdog-BG][${requestId}] Task 10 (Recontext Reframe Check) failed:`, e.message);
    }
    try {
      console.log(`[Watchdog-BG][${requestId}] === Task 11: Recovering Stalled Reframe Jobs (Processing) ===`);
      await recoverStalledJobs('mira-agent-jobs', [
        'processing'
      ], STALLED_REFRAME_THRESHOLD_SECONDS, 'MIRA-AGENT-worker-reframe', 'id', 'job_id');
    } catch (e) {
      console.error(`[Watchdog-BG][${requestId}] Task 11 (Stalled Processing Reframe) failed:`, e.message);
    }
    try {
      ...
      console.error(`[Watchdog-BG][${requestId}] Task 12 (Stalled Awaiting Reframe) failed:`, e.message);
    }
    try {
    ...
    try {
      ...
      console.error(`[Watchdog-BG][${requestId}] Task 15 (Report Chunks) failed:`, e.message);
    }
    try {
      ...
      console.error(`[Watchdog-BG][${requestId}] Task 16 (Synthesis) failed:`, e.message);
    }
    try {
      ...
      await recoverStalledJobs('mira-agent-bitstudio-jobs', [
        'awaiting_fix',
        'fixing'
      ], STALLED_FIXER_THRESHOLD_SECONDS, 'MIRA-AGENT-fixer-orchestrator', 'id', 'job_id');
    } catch (e) {
      console.error(`[Watchdog-BG][${requestId}] Task 17 (Stalled Fixer) failed:`, e.message);
    }
    try {
      console.log(`[Watchdog-BG][${requestId}] === Task 18: Recovering Stalled QA Reports ===`);
      await recoverStalledJobs('mira-agent-vto-qa-reports', [
        'processing'
      ], STALLED_QA_REPORT_THRESHOLD_SECONDS, 'MIRA-AGENT-worker-vto-reporter', 'id', 'qa_job_id');
    } catch (e) {
      console.error(`[Watchdog-BG][${requestId}] Task 18 (Stalled QA) failed:`, e.message);
    }
    try {
      console.log(`[Watchdog-BG][${requestId}] === Task 19: Recovering Stalled Report Chunks ===`);
      await recoverStalledJobs('mira-agent-vto-report-chunks', [
        'processing'
      ], STALLED_CHUNK_WORKER_THRESHOLD_SECONDS, 'MIRA-AGENT-analyzer-vto-report-chunk-worker', 'id', 'chunk_id');
    } catch (e) {
      console.error(`[Watchdog-BG][${requestId}] Task 19 (Stalled Chunks) failed:`, e.message);
    }
    try {
      ...
      if (awaitingFallbackJobs && awaitingFallbackJobs.length > 0) {
        const fallbackCheckPromises = awaitingFallbackJobs.map(async (vtoJob)=>{
          ...
          if (bitstudioJob?.status === 'complete') await supabase.functions.invoke('MIRA-AGENT-worker-vto-pack-item', {
            body: {
              pair_job_id: vtoJob.id,
              bitstudio_result_url: bitstudioJob.final_image_url
            }
          });
          else if (bitstudioJob?.status === 'failed' || bitstudioJob?.status === 'permanently_failed') await supabase.from('mira-agent-bitstudio-jobs').update({
            status: 'failed',
      ...
      console.error(`[Watchdog-BG][${requestId}] Task 20 (BitStudio Fallback) failed:`, e.message);
    }
    try {
      ...
      if (awaitingAutoCompleteJobs && awaitingAutoCompleteJobs.length > 0) {
        const autoCompleteCheckPromises = awaitingAutoCompleteJobs.map(async (parentJob)=>{
          ...
          if (childJob?.status === 'complete') await supabase.from('mira-agent-bitstudio-jobs').update({
            status: 'complete',
          ...
          else if (childJob?.status === 'failed' || childJob?.status === 'permanently_failed') await supabase.from('mira-agent-bitstudio-jobs').update({
            status: 'failed',
      ...
      console.error(`[Watchdog-BG][${requestId}] Task 21 (Auto-Complete Check) failed:`, e.message);
    }
    try {
      console.log(`[Watchdog-BG][${requestId}] === Task 22: Recovering Stalled Stylist Jobs ===`);
      await recoverStalledJobs('mira-agent-bitstudio-jobs', [
        'awaiting_stylist_choice'
      ], STALLED_STYLIST_CHOICE_THRESHOLD_SECONDS, 'MIRA-AGENT-stylist-chooser', 'id', 'pair_job_id');
    } catch (e) {
      console.error(`[Watchdog-BG][${requestId}] Task 22 (Stalled Stylist) failed:`, e.message);
    }
