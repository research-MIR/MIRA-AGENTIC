...
import { Shirt, Plus, Loader2 } from "lucide-react";
...
const WardrobePacks = () => {
  ...
  const { data: packs, isLoading, error } = useQuery<GarmentPack[]>({
    queryKey: ["garmentPacks", session?.user?.id],
    queryFn: async () => {
      if (!session?.user) return [];
      const { data, error } = await supabase
        .from("mira-agent-garment-packs")
  ...
  const handleCreatePack = async () => {
    ...
    setIsCreating(true);
    const toastId = showLoading("Creating garment pack...");
    try {
      const { error } = await supabase.from('mira-agent-garment-packs').insert({
        name: newPackName,
      ...
      queryClient.invalidateQueries({ queryKey: ['garmentPacks'] });
  ...
  return (
    <div className="p-4 md:p-8 h-screen overflow-y-auto">
      <header className="pb-4 mb-8 border-b flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">{t('wardrobePacksTitle')}</h1>
          <p className="text-muted-foreground">{t('wardrobePacksDescription')}</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {packs.map(pack => (
            <Link key={pack.id} to={`/wardrobe-packs/${pack.id}`}>
        ...
        <div className="text-center py-16">
          ...
          <h2 className="mt-4 text-xl font-semibold">{t('noGarmentPacksTitle')}</h2>
          <p className="mt-2 text-muted-foreground">{t('noGarmentPacksDescription')}</p>
        </div>
      )}
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('createNewGarmentPack')}</DialogTitle>
            <DialogDescription>{t('createNewGarmentPackDescription')}</DialogDescription>
          </DialogHeader>
