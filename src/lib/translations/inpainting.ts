export const inpaintingTranslations = {
  it: {
    inpainting: "Inpainting",
    inpaintingDescription: "Modifica parti specifiche di un'immagine con un controllo a livello di pixel.",
    ...
    vtoHelpTitle: "Guida all'Inpainting",
    vtoHelpIntro: "Questa guida spiega le diverse modalità e impostazioni disponibili nella scheda Inpainting.",
    ...
    proSettingsTooltip: "Sblocca impostazioni avanzate come Intensità Denoise, Espansione Maschera e Alta Risoluzione per perfezionare i tuoi risultati.",
...
- **Se Scrivi Manualmente:** Se preferisci il controllo manuale, ricorda che stai sostituendo il lavoro dell'IA. Il tuo prompt deve descrivere in dettaglio l'oggetto da inserire *come se fosse nella scena*.
  ...
  - **❌ Esempio negativo:** "una donna che tiene in mano una tazza" (Descrive l'intera scena)
  - **✅ Esempio positivo:** "una tazza di ceramica bianca fotorealistica con un leggero riflesso della finestra, tenuta in mano" (Descrive solo l'oggetto e il suo contesto di illuminazione)

#### 2. La Maschera è Tutto: Strategie di Selezione
- **Sii Generoso:** È meglio mascherare un'area leggermente più grande del necessario. Questo aiuta l'IA ad avere più spazio per fondere il nuovo contenuto con l'originale.
- **Usa l'Espansione:** L'impostazione "Espansione Maschera" è la tua migliore amica. Aumentala per ammorbidire i bordi e ottenere una transizione più naturale, evitando contorni netti.
- **Strategie di Copertura:**
  1.  **Sostituzione di un oggetto:** Assicurati di coprire completamente l'intero oggetto originale per rimuoverlo del tutto.
  2.  **Aggiunta di un oggetto:** Maschera l'area in cui vuoi che il nuovo oggetto appaia.
  3.  **Modifica di una parte:** Se stai modificando solo una parte di un oggetto (es. cambiare il colore di un'auto), maschera solo quella parte.

#### 3. L'Immagine di Riferimento (Opzionale)
- **È per l'Oggetto, non per lo Stile Generale:** A differenza di altri strumenti, qui l'immagine di riferimento serve a mostrare all'IA l'**oggetto o la texture specifica** che vuoi inserire nell'area mascherata. L'IA cercherà di replicare l'oggetto di riferimento.
- **Esempio:** Se fornisci l'immagine di un orologio specifico e maschera il polso di una persona, l'IA tenterà di ricreare *quell'orologio* nell'area che hai disegnato, adattandolo alla posa e all'illuminazione.
...
- **Controlla la libertà dell'IA:** L'impostazione "Intensità Denoise" determina quanto l'IA può deviare dall'immagine originale all'interno della maschera.
  - **Basso Denoise (es. 0.5-0.7):** Ideale per cambi di texture o colore, o quando la forma dell'oggetto originale è simile a quella desiderata. Aiuta a fondere meglio il risultato.
  - **Alto Denoise (es. 0.8-1.0):** Concede all'IA più libertà creativa. Usalo se vuoi cambiare significativamente la forma di un oggetto o se l'impostazione più bassa non è abbastanza forte.
...
`
  ...
  en: {
    inpainting: "Inpainting",
    inpaintingDescription: "Edit specific parts of an image with pixel-level control.",
    proMode: "Pro Mode",
    ...
    vtoHelpTitle: "Inpainting Guide",
    vtoHelpIntro: "This guide explains the different modes and settings available in the Inpainting tab.",
    ...
    proSettingsTooltip: "Unlock advanced settings like Denoise Strength, Mask Expansion, and High Resolution to fine-tune your results.",
...
Inpainting is a powerful tool for replacing or modifying parts of an image, but it requires a specific approach to get the best results.

#### 1. The Precise Prompt: Let the AI Do the Work
- **Use Auto-Prompt (Recommended):** If you provide a reference image, we highly recommend leaving the "Auto-Generate" switch ON. The AI will analyze your source and reference images to create a technically perfect prompt that describes how the new object/texture should appear within the context of the original scene (lighting, perspective, etc.).
- **If Writing Manually:** If you prefer manual control, remember that you are overriding the AI's work. Your prompt must describe the object to be inserted in detail *as if it were in the scene*.
  ...
  - **❌ Bad Example:** "a woman holding a cup" (Describes the whole scene)
  - **✅ Good Example:** "a photorealistic white ceramic mug with a slight window reflection, held in a hand" (Describes only the object and its lighting context)

#### 2. The Mask is Everything: Selection Strategies
- **Be Generous:** It's often better to mask a slightly larger area than you think you need. This gives the AI more room to blend its creation with the original.
- **Use Expansion:** The "Mask Expansion" setting is your best friend. Increase it to soften the edges and achieve a more natural transition, avoiding hard outlines.
- **Coverage Strategies:**
  1.  **Replacing an object:** Make sure to completely cover the entire original object to remove it completely.
  2.  **Adding an object:** Mask the area where you want the new object to appear.
  3.  **Modifying a part:** If you're only changing part of an object (e.g., changing a car's color), mask only that part.

#### 3. The Reference Image (Optional)
- **It's for the Object, Not the General Style:** Unlike other tools, the reference image here is used to show the AI the **specific object or texture** you want to insert into the masked area. The AI will try to replicate the reference object.
- **Example:** If you provide an image of a specific watch and mask a person's wrist, the AI will attempt to recreate *that watch* in the area you've drawn, adapting it to the pose and lighting.
