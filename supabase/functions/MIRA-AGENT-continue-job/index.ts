...
serve(async (req) => {
  ...
  try {
    const { jobId, prompt, storagePaths, isDesignerMode, pipelineMode } = await req.json();
    ...
    const newContext = { ...job.context, history, isDesignerMode, pipelineMode };

    console.log(`[ContinueJob][${jobId}] Updating job history and setting status to 'processing'.`);
