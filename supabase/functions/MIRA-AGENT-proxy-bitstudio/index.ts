...
const fetchWithRetry = async (url: string, options: any, requestId: string, maxRetries = 3) => {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (response.status === 429) {
        const delay = [7000, 10000, 15000][attempt - 1] || 15000;
        console.log(`[FetchRetry][${requestId}] Rate limit hit. Waiting ${delay}ms before retrying...`);
        await new Promise(resolve => setTimeout(resolve, delay));
        continue;
      } else if (response.status >= 500 && attempt < maxRetries) {
        const delay = 1500 * attempt; // 1.5s, 3s, 4.5s
        console.log(`[FetchRetry][${requestId}] Server error. Waiting ${delay}ms before retrying...`);
        await new Promise(resolve => setTimeout(resolve, delay));
        continue;
      } else {
        // For 4xx errors (other than 429) or after max retries
...
async function uploadToBitStudio(fileBlob: Blob, type: any, filename: string, requestId: string) {
  ...
  const response = await fetchWithRetry(`${BITSTUDIO_API_BASE}/images`, {
    method: 'POST',
    headers: {
  ...
  if (!result.id) throw new Error(`BitStudio upload for type ${type} did not return an ID.`);
  return result.id;
}
    ...
    if (retry_job_id) {
      ...
      const { error: updateError } = await supabase.from('mira-agent-bitstudio-jobs').update({
        ...
        metadata: {
          ...jobToRetry.metadata,
          engine: 'bitstudio',
          prompt_used: retryPayload.prompt,
          retry_count: (jobToRetry.metadata.retry_count || 0) + 1
        },
        error_message: null,
      ...
      if (mode === 'inpaint') {
        ...
        if (batch_pair_job_id) {
          const { data: existingJob, error: checkError } = await supabase.from('mira-agent-bitstudio-jobs').select('id').eq('batch_pair_job_id', batch_pair_job_id).not('status', 'in', '(failed,permanently_failed)').maybeSingle();
