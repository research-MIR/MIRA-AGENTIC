...
const Index = () => {
  ...
  const processJobData = useCallback((jobData: any) => {
    console.log('[processJobData] Processing job update. Full data:', JSON.stringify(jobData, null, 2));
    if (!jobData) {
        console.warn('[processJobData] Job data is null or undefined. Aborting processing.');
        return;
    }
    
    console.log(`[processJobData] Job status is: ${jobData.status}`);
    const newIsJobRunning = jobData.status === 'processing' || jobData.status === 'awaiting_refinement';
    console.log(`[processJobData] UI 'isJobRunning' will be set to: ${newIsJobRunning}`);
  ...
  // Fallback Poller: This is a safety net in case a Realtime message is missed.
  useEffect(() => {
    let poller: number | null = null;

    // Only start polling if the job is in a state that should be temporary.
    const shouldPoll = isJobRunning || (jobData?.status === 'processing' || jobData?.status === 'awaiting_refinement');
    
    if (jobId && shouldPoll) {
        console.log(`[Fallback Poller] Job is in a processing state (${jobData?.status}). Starting fallback poller.`);
        poller = setInterval(() => {
            console.log('[Fallback Poller] Polling for job update...');
    ...
    return () => {
        if (poller) {
            console.log('[Fallback Poller] Clearing poller.');
            clearInterval(poller);
        }
    };
  }, [isJobRunning, jobData?.status, jobId, queryClient]);
