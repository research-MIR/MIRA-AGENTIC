...
const getDynamicSystemPrompt = (jobContext: any): string => {
    ...
    let basePrompt = `You are Mira, a master AI orchestrator. Your job is to create and execute a multi-step plan to fulfill the user's request. You will be given the full history of the conversation. Your response MUST ALWAYS be a tool call.

**CRITICAL LANGUAGE INSTRUCTION:** You MUST generate the final user-facing summary for the \`finish_task\` tool in the following language: **${language}**. All other internal reasoning and tool calls should remain in English.
...
2.  **Text-to-Image Workflow:**
    ...
    if (jobContext?.ratioMode && jobContext.ratioMode !== 'auto') {
        basePrompt += `\n- **User Preference:** The user has specified a desired aspect ratio of **${jobContext.ratioMode}**. You MUST use this ratio in your \`generate_image\` tool call.`;
    }
    if (jobContext?.numImagesMode && jobContext.numImagesMode !== 'auto') {
        basePrompt += `\n- **User Preference:** The user has specified they want **${jobContext.numImagesMode}** image(s). You MUST use this number in your \`generate_image\` tool call.`;
    }
...
You MUST NOT answer the user directly. Your only output is a tool call.`;
    ...
    console.log(`[MasterWorker] Using dynamic system prompt for pipeline_mode: '${pipelineMode}' and language: '${language}'`);
    return finalPrompt;
};
